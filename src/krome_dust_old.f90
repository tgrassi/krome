...
#KROME_opt_integral
  ...
  !***********************
  !this sub tabulates the B(E,Td)*Q(a,E) integral as a function
  ! of Td for each dust bin. Arguments
  ! dust_opt_Em is the table (eV/cm2/s, out)
  ! dust_opt_Tbb is the array of the Tbb evaluated (out, K)
  ! dust_opt_asize is the array of the asizes (in,cm)
  ! dust_opt_nu frequency array (in, eV)
  ! dust_opt_Qabs opacity table (in, cm2/g)
...
!!$    end do
  ...
  function dustEm(asize, Tbb, dust_opt_asize, dust_opt_nu, dust_opt_Qabs)
    ...
    !computes integral by using trapezoidal method
    do i=2,size(dust_opt_nu)
       kera = planckBB(dust_opt_nu(i-1), Tbb) !Planck function is the kernel
       kerb = planckBB(dust_opt_nu(i), Tbb) !Planck function is the kernel
       dnu = dust_opt_nu(ival) - dust_opt_nu(ival-1) !stepsize
       !calculates integrals
       int0 = int0 + 0.5d0 * (kera * dust_opt_Qabs(ival-1,i-1) &
            + kerb * dust_opt_Qabs(ival-1,i)) * dnu
  ...
  subroutine init_Qabs(fname,opt_Qabs,opt_asize,opt_nu)
    ...
    do
       ...
       if(icount.le.size(opt_nu)) opt_nu(icount) = x(2) * planck_eV
